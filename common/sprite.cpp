...
void Sprite::SetUp(GLuint vb, GLuint uvb, GLuint texture, unsigned int width, unsigned int height)
{
	if (setup)
	{
		return;
...
GLuint Sprite::loadTGA(const std::string& imagepath)
{
	...
	_texture = textureID; //if you delete this yo sprite becomes BLACK >:(
	return textureID;
}
